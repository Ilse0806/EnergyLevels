...
import com.example.microhabits.services.CreatorService.saveNewRecipe
...
@Composable
fun CreateFoodScreen(navController: NavController) {
    ...
    Scaffold(
        ...
        if (showSuccessOverlay) {
            ...
            saveNewRecipe(VariableModel.newRecipe.value.toMap(), context)
            if (show) {
                SuccessOverlay(
                    onGoHome = { navController.navigate(route = Home) },
                    onViewGoal = {
                        navController.navigate(route = FoodDetails(1))
                    },
                    modifier = Modifier
                        .fillMaxSize()
                        .zIndex(1f),
        ...
        Column (
            ...
            ActivityEditor("recipe", { showSuccessOverlay = true })
        }
    }
}
